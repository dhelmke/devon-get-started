// Please include this preamble in every page!
:toc: macro
toc::[]
:idprefix:
:idseparator: -
ifdef::env-github[]
:tip-caption: :bulb:
:note-caption: :information_source:
:important-caption: :heavy_exclamation_mark:
:caution-caption: :fire:
:warning-caption: :warning:
endif::[]

= Home
// The '=' denotes the document title. It should match the filename and must not be used for headings!
// For more information visit: https://asciidoctor.org/docs/asciidoc-syntax-quick-reference/

== Overview
A short link:Overview[**Overview**] of the information sources which are there for devon.

== Introduction
The first link:Introduction[**Introduction**] of getting to know devon.

== IDE setup
How easy can the link:IDE-setup[**IDE setup**] be?

== JumpTheQueue tutorial 

== My-Thai-Star setup

== Further information